...
> **修复闭环更新（2026-07-31 17:20）**：线上版本已到 `91f248e`。同一组 12 份差异化简历在唯一 PARTIAL 单例重跑后 **12/12 SUCCESS**；Runtime P95 202.23s→127.24s，ReportAgent P95 154.50s→81.78s，平均报告字符仅 5,688→5,600，MCP 11/11 SUCCESS。知识库与岗位匹配 RAG 均已完成带标签评测；`/data` 已从 100% 降至 37%。第 8 节为最终闭环状态，上表仍保留原始 100 份压测快照。
...
|:---:|---|---|:---:|---|
| P0 | 深评消费吞吐低于入口 | 1 QPS 入站与 0.0858 份/s 深评完成吞吐是两个口径；单 ECS + 当前 provider 资源下无法持续 1 QPS 完成深评。 | **容量边界已闭环** | 验收结论是“1 QPS 异步上传可承接、100 份突发可排空”，不伪装成“1 QPS 深评完成吞吐”。若要持续 1 QPS 深评，需增加 provider 额度/实例并水平扩容，不再在单机参数上假优化。 |
