...
- **2026-07-29 ECS 全库真实回放**：脚本增加 `--mysql-container` 数据源，绕过 Ops API 最近 200 条的分页上限；全量输入 764 条 / 68 个 run，764 条全部有效、0 条负年龄。其中 48 条旧存储类型按与 `MemoryService` 一致的口径归一：`CONVERSATION → WORKING`、`PREFERENCE → SEMANTIC`。Working USED=11 / 最大 0.435 天；Semantic=37 / 5.857 天；Episodic=424 / 3.441 天；Procedural=292 / 0.781 天。四类都未达到当前 TTL 80% 的时间覆盖门，所有缩短候选在现有时间窗中均为 100% 留存，无法分辨未来会不会误删后续需要的 memory。实验决策仍为 `KEEP_CURRENT_DEFAULTS_INSUFFICIENT_DATA`，因此代码中的生产配置 **Working 2 / Semantic 90 / Episodic 90 / Procedural 365 天继续生效**；不把“短历史内都没掉分”伪装成“最优 TTL”。产物：`reports/experiments/memory_ttl_replay.{json,md}`。
