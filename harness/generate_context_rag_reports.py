...
def coordinator_prompt_reference_card() -> str:
    ...
    """
    initial_user_template = """任务类别: {run_type}
...
输出 JSON {"plan": [...], "reason": "基于[具体信号]选择了[具体策略]"}"""
    replan_user_template = """运行中触发重规划，原因: {trigger}
已完成 Agent: {executed}
剩余计划: {remaining}
...
只允许调整剩余部分（不得包含已完成 Agent，handoff 不得形成环），必须满足 requires_artifacts，最后一个必须是唯一 terminal Agent。若当前剩余计划已合理，原样返回。输出 JSON {"plan": [...], "reason": "..."}"""
    return (
        ...
        f"{_details('Coordinator user message：初始 plan 的完整组装模板（_refine，本轮未发送）', _text_fence(initial_user_template))}\n\n"
        f"{_details('Coordinator user message：动态 replan 的完整组装模板（本轮三次 gate 均未触发发送）', _text_fence(replan_user_template))}\n\n"
        ">为什么这里只能展示组装模板：本轮 `full_evaluation` 在进入 `_refine()` 前直接返回确定性 base plan，"
        "因此进程里没有创建这两个 `messages[1].content`，审计库也不可能存在一份真实 Provider user message。"
        "第 4 节列出的本轮 planner 参数和计划 JSON 才是本次实际输入/输出。\n\n"
        "</details>"
    )
