...
### 2.3 我们的"图"是每次运行动态生成的，而且运行中会变

- Coordinator 按 runType 规则 + LLM 精化产出计划与并行分组；运行中还有三种变化：
  失败重排（replan_after_failure）、自适应重规划（触发条件：组失败/新冲突/低置信）、
  agent handoff 插队。
- LangGraph 表达这种"计划即数据"的模式要用条件边 + 动态子图组装，图结构本身
  变成运行时数据——此时静态图 DSL 不再提供任何抽象收益，反而多一层间接。
- 自研后：计划就是 `List[List[str]]`，重规划就是列表操作 + 依赖拓扑校验，
  40 行核心逻辑，trace 事件直接可见。
