...
> - Java `RUN_MAX_GLOBAL_CONCURRENT=12` permit 覆盖完整 workflow，保证 active Run 不突破硬上限；Python `LLM_MAX_CONCURRENT=64` 独立限制每次供应商调用。LLM await 会挂起协程而不阻塞事件循环，但不会提前释放完整 Run permit。
